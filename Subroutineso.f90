...
!===== SUBROUTINE FOR CHOLESKY DECOMPOSITION OF PENTADIAGONAL MATRICES
 ...
 integer(kind=int32),intent(in) :: is,ie
 real(kind=dp),dimension(0:lim,3),intent(inout) :: xu
 real(kind=dp),dimension(0:lim,2),intent(inout) :: xl
 real(kind=dp),dimension(-2:2,0:2),intent(in) :: albes,albee
...
!===== SUBROUTINE FOR BOUNDARY FILTER COEFFICIENTS
 ...
 real(kind=dp),intent(in) :: fltk,fltkbc
 real(kind=dp),dimension(-2:2,0:2),intent(inout) :: albef
 real(kind=dp),dimension(0:2),intent(inout) :: fa,fb,fc
 real(kind=dp) :: alphz,betz,za,zb,zc
...
!===== SUBROUTINE FOR INTERIOR FILTER COEFFICIENTS
 ...
 real(kind=dp),intent(in) :: fltk,fltr
 real(kind=dp),intent(inout) :: alphz,betz,za,zb,zc
 real(kind=dp),dimension(3) :: cosf
    ...
    za=60*(1-fltr)*cos(half*fltk)**4*fctr; zb=-0.4*za; zc=za/15
...
!===== SUBROUTINE FOR SUBDOMAIN-BOUNDARY COEFFICIENTS
 ...
 real(kind=dp),dimension(:,:),allocatable :: ax,bx,rx,sx
 real(kind=dp),dimension(0:4) :: zv
 real(kind=dp) :: alphz,betz,za,zb,zc
...
!===== SUBROUTINE FOR MATRIX INVERSION
 ...
 integer(kind=int32),intent(in) :: is,ie
 real(kind=dp),dimension(is:ie,is:ie),intent(in) :: ax
 real(kind=dp),dimension(is:ie,is:ie),intent(inout) :: sx

 integer(kind=int32),dimension(1) :: imax
 integer(kind=int32),dimension(is:ie) :: ipvt
 real(kind=dp),dimension(is:ie,is:ie) :: rx
 real(kind=dp),dimension(is:ie) :: temp
...
!===== SUBROUTINE FOR MOVING FRAME VELOCITIES
 ...
 real(kind=dp),intent(in) :: dtko,dtk
...
!===== SUBROUTINE FOR BLASIUS LAMINAR BOUNDARY LAYER
 ...
 integer(kind=int32),intent(in) :: lbl
 real(kind=dp),dimension(0:lbl),intent(in) :: x,y
 real(kind=dp),dimension(0:lbl),intent(inout) :: blu,blv,blm

 real(kind=dp) :: blas,spr,eta,etb

    blas=0.3320573362151963; spr=sqrt(prndtl)
    ra0=half*pi; ra1=3*blas*blas/560; ra2=11*blas/420
 do i=0,lbl
    fctr=1/sqrt(x(i))
    eta=fctr*sqrtrema*y(i); etb=spr*eta
    res=0.000001*eta**4*exp(eta**1.3625)
    blu(i)=(blas*eta+ra1*eta**4+res)/(1+ra2*eta**3+res)
    blv(i)=0.8604*fctr*sqrtremai*(sin(ra0*tanh(exp(0.177*eta)-1)))**1.96
    res=0.000001*etb**4*exp(etb**1.3625)
...
!===== SUBROUTINE FOR GRID LINE GENERATION
 ...
 integer(kind=int32),intent(in) :: lxi,mxin,ip
 real(kind=dp),dimension(0:lxi),intent(inout) :: x,xxi
 real(kind=dp),intent(in) :: xo,xn,dxo,dxn

 integer(kind=int32) :: i,ii
 real(kind=dp) :: dxoo,dxnn,aa,bb,cc,ee,dd,xi,fctr

    dxoo=dxo; dxnn=dxn
 if(int((dxo+sml)/free)==1) then
    dxoo=2*(xn-xo)/mxin-dxnn
 end if
 if(int((dxn+sml)/free)==1) then
    dxnn=2*(xn-xo)/mxin-dxoo
 end if
...
!
! integer(kind=int32),intent(in) :: ns,lxi,mxic,mxin,ip
! real(kind=dp),dimension(0:lxi),intent(inout) :: x,xxi
! real(kind=dp),intent(in) :: xo,xn,dxs,am
!
! integer(kind=int32) :: i,ii
! real(kind=dp) :: amp0,amm0,alp0,alp1,s0,s1,c0,c1,aa,bb,xi,xic,xin,xii
!
!    amp0=am+1; amm0=am-1
!    xic=mxic; xin=mxin
...
!===== SUBROUTINE FOR CHARACTER STRING CONVERSION
 ...
 integer(kind=int32),intent(in) :: nfile
 integer(kind=int32),intent(inout) :: lh
...
!===== FUNCTION FOR MAIN INDEX TRANSFORMATION IN 3D
 ...
 integer(kind=int32),intent(in) :: i,j,k,nn
 integer(kind=int32) :: lm
...
!===== FUNCTION FOR MAIN INDEX TRANSFORMATION IN 2D
 ...
 integer(kind=int32),intent(in) :: i,j,nn
 integer(kind=int32) :: lm
