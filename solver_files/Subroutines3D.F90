...
!===== SUBROUTINE FOR MPI IMPLEMENTATION
 ...
 integer(kind=int32),intent(in) :: nt,nrt,n45,itag
...
!===== SUBROUTINE FOR COMPACT FINITE DIFFERENTIATING
 ...
 integer(kind=int32),intent(in) :: nn,nz,m
...
!===== SUBROUTINE FOR COMPACT FILTERING
 ...
 integer(kind=int32),intent(in) :: nn,nz
...
!===== SUBROUTINE FOR ELEMENTARY VARIABLES IN GCBC/GCIC
 ...
 real(kind=dp),dimension(3),intent(in) :: cm
...
!===== SUBROUTINE FOR GENERATING TECPLOT DATA FILE
 ...
 integer(kind=int32),intent(in) :: nf,n,mb
 ...
 if(mb==0) then
    ...
    write(nf,pos=4*lh+1) int(min(n+2,2),kind=int16); lh=lh+1 ! File Type (0 = Full / 1 = Grid / 2 = Solution)
    cinput=cfilet(n); call strio(nf,lh,cinput) ! File Title
    write(nf,pos=4*lh+1) int(mq,kind=int16); lh=lh+1 ! Number of Variables
 if(n==-1) then
 ...
 do mm=0,mbk
    ...
    write(nf,pos=4*lh+1) int(n+1,kind=int16); lh=lh+1 ! Strand ID
    write(nf,pos=4*lh+1) real(times(max(n,0)),kind=dp); lh=lh+2 ! Solution Time (Double)
    ...
    write(nf,pos=4*lh+1) int(lximb(mm)+1,kind=int16); lh=lh+1 ! IMax
    write(nf,pos=4*lh+1) int(letmb(mm)+1,kind=int16); lh=lh+1 ! JMax
    write(nf,pos=4*lh+1) int(lzemb(mm)+1,kind=int16); lh=lh+1 ! KMax
...
!===== SUBROUTINE FOR FINDING VARIABLE MIN/MAX VALUES FOR TECPLOT DATA FILE
 ...
 integer(kind=int32),intent(in) :: nn
