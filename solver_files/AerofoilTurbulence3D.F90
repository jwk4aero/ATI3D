...
!===== SUBINPUT PARAMETERS
 ...
 open(9,file='inputp.dat' OPENFILESHARED)
     ...
     open(9,file='randnum.dat' OPENFILESHARED)
     do m=1,3
...
!===== DOMAIN DECOMPOSITION & BOUNDARY INFORMATION
 ...
 subroutine domdcomp
     ...
     select case(mb)
         case(0,3); ms(1)=mb+2; me(1)=mb+1
         case(1,4); ms(1)=mb-1; me(1)=mb+1
         case(2,5); ms(1)=mb-1; me(1)=mb-2
     end select

     ! set neighboring block in y
     select case(mb)
         case(0,1,2); ms(2)=mb+3; me(2)=mb+3
         case(3,4,5); ms(2)=mb-3; me(2)=mb-3
     end select
     ms(3)=mb; me(3)=mb
...
#else ! VORTEX_INIT
     do l=0,lmx
         uu=cf1**2*((ss(l,1)-xcore)**2+ss(l,2)**2)
         expn=cf2*exp(half*(1-uu))/(2*pi)

         qa(l,1)=rhooo*(1-half*gamm1*expn**2)**hamm1
         qa(l,2)=qa(l,1)*(qa(l,2)+aoo*expn*cf1*ss(l,2))
         qa(l,3)=qa(l,1)*(qa(l,3)-aoo*expn*cf1*(ss(l,1)-xcore))
         qa(l,4)=0
         p(l)=poo*qa(l,1)**gam
         qa(l,5)=hamm1*p(l)+half*(qa(l,2)**2+qa(l,3)**2+qa(l,4)**2)/qa(l,1)
     end do
#endif ! VORTEX_INIT
