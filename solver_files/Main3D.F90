#ifdef FOPEN_NOT_BINARY
#define OPENFILEFORM 'unformatted'
#else ! not FOPEN_NOT_BINARY
#define OPENFILEFORM 'binary'
#endif ! not FOPEN_NOT_BINARY

#ifdef FOPEN_NOT_SHARED
#define OPENFILESHARED
#else ! not FOPEN_NOT_SHARED
#define OPENFILESHARED ,shared
#endif ! not FOPEN_NOT_SHARED
...
#ifdef SUBSPACES
!cpb   subspace environment
       integer ss_index,ndim,ialloc
#endif ! SUBSPACES
...
!===== PREPARATION FOR PARALLEL COMPUTING
     ...
     inquire(iolength=ll) real(1.0,kind=sp); nrec=ll

!===== INPUT PARAMETERS

     open(9,file='inputo.dat' OPENFILESHARED)
     read(9,*) cinput,mbk
     read(9,*) cinput,nts
     ...
     srefoo=111.0/tempoo; srefp1dre=(srefoo+1)/reoo; sqrtrema=sqrt(reoo*amachoo); sqrtremai=1/sqrtrema
...
!===== DOMAIN DECOMPOSITION & BOUNDARY INFORMATION
    ...
    !set mo as start index of first process IDs in block
     mo(0)=0
     do mm=1,mbk
        mo(mm)=mo(mm-1)+npc(mm-1,1)*npc(mm-1,2)*npc(mm-1,3)
     end do

     !set mb to block ID of current process
     do mm=0,mbk
...
!cpb create communicator for each block respectively
     ...
     if (ierr .ne. 0) then
         write(*,*) 'Error ',ierr,' in MPI_COMM_SPLIT!'
     ...
     ! call case specific domain decomposition
     call domdcomp

     ! set process coordinates (ip,jp,kp / temporal) in current block (starting from 0)
     ...
     do nn=1,3
         ...
         ! number of processes for current block in current direction
         ma=npc(mb,nn)
         ! if only one process then simply copy conditions
         if(ma==1) then
            l=ll; nbc(0,nn)=nbcs(nn); nbc(1,nn)=nbce(nn); ncd(0,nn)=ncds(nn); ncd(1,nn)=ncde(nn)
         end if
         ! multiple processes
         if(ma>=2) then
             ! first process of respective direction
             if(lp==0) then
                 ! set number of points
                 l=ll-((ll+1)/ma)*(ma-1)
                 ! set number of points
                 nbc(0,nn)=nbcs(nn); nbc(1,nn)=40; ncd(0,nn)=ncds(nn); ncd(1,nn)=myid+mp
             end if
             ! set number of points
             if(lp>0.and.lp<ma-1) then
                 ...
                 l=(ll+1)/ma-1
                 ! set conditions
                 nbc(0,nn)=40; nbc(1,nn)=40; ncd(0,nn)=myid-mp; ncd(1,nn)=myid+mp
             end if
             ! last process of respective direction
             if(lp==ma-1) then
                 ! set number of points
                 l=(ll+1)/ma-1
                 ! set conditions
                 nbc(0,nn)=40; nbc(1,nn)=nbce(nn); ncd(0,nn)=myid-mp; ncd(1,nn)=ncde(nn)
             end if
         end if
         select case(nn)
             ! store number of points
             case (1); lxi=l; case (2); let=l; case (3); lze=l
         end select
     end do

!===== SUBDOMAIN SIZES & WRITING START POSITIONS IN OUTPUT FILE

     ! master process (0) collects and broadcasts all processor sizes (points)
     ! collection of lxi,let,lze
     if(myid==0) then
     ...
     ! setup start index of block processor in global array
     do mm=0,mbk
         ! setup first process
         lpos(mo(mm))=0
         ! setup all processes with j=k=0
         do i=1,npc(mm,1)-1
            mp=mo(mm)+i
            lpos(mp)=lpos(mp-1)+lxim(mp-1)+1
         end do
         ! setup all processes with j!=0 k=0
         jp=npc(mm,1)
...
!===== GRID INPUT & CALCULATION OF GRID METRICS
 ...
 do j=0,let; jp=j*(lxio+1)
    ...
    open(9,file=cgrid,access='stream',form=OPENFILEFORM OPENFILESHARED)
    lp=lpos(myid)
...
!===== SETUP SUBSPACES
     ...
     ndim=0
    !cpb   set subspace flag for NSRHS to false if no subspace
    !cpb   output is performed by the respective proc
     ss_flag = .false.
     do ss_index=1,ss_num
    !cpb     set subspace flag to true if subspace output is performed by proc
       ss_flag = .true.
       ndim = ndim + ss_dim_proc(ss_index,1) * ss_dim_proc(ss_index,2) &
                   * ss_dim_proc(ss_index,3) * ss_num_var(ss_index)
       ss_first_write(ss_index)=.false.
     enddo
     allocate (ss_qio(ndim),stat=ialloc)
     if(ialloc .ne. 0)then
       write(*,*)'allocate error:',ialloc
       call MPI_FINALIZE(ierr)
       stop
     endif
#endif
     if(myid.eq.0) write(*,*) 'Subspaces initialized!'
     ! write subspace grid files
     call subspace_write_grid()
     if(myid.eq.0) write(*,*) 'Subspaces grids written!'
#endif ! SUBSPACES
...
!===== INITIAL CONDITIONS
 ...
 if(nts==0) then
    ...
    open(9,file=crestart,access='stream',form=OPENFILEFORM OPENFILESHARED); lh=0
...
!----- DETERMINATION OF TIME STEP SIZE & OUTPUT TIME
 ...
 if(nviscous==1) then
    ...
    dt=dts+(dte-dts)*sin(0.05*pi*(n-ndt))**2
...
!==========================
 ...
 if(myid==0) then
    ...
    write(9,'(2es15.7)') real(npro,kind=dp),wtime/npro
    close(9)
 end if
...
!===== GENERATING RESTART DATA FILE
 ...
 if(myid==mo(mb)) then
    ...
    open(9,file=crestart,access='stream',form=OPENFILEFORM OPENFILESHARED); lh=0
 if(myid==mo(mb)) then
    write(9,pos=8*lh+1) n; lh=lh+1
    write(9,pos=8*lh+1) ndt; lh=lh+1
    write(9,pos=8*lh+1) dt; lh=lh+1
    write(9,pos=8*lh+1) dts; lh=lh+1
    write(9,pos=8*lh+1) dte; lh=lh+1
    write(9,pos=8*lh+1) timo; lh=lh+1
 else
    lh=lh+6
 end if
    lp=lpos(myid)+lh
 do m=1,5; lq=(m-1)*ltomb
 do k=0,lze; do j=0,let; l=indx3(0,j,k,1)
    write(9,pos=8*(lp+lq+lio(j,k))+1) qa(l:l+lxi,m)
 end do; end do
 end do
    close(9)
 end if
...
!crp     is waited for the last writing to finish
            ...
            call MPI_TYPE_FREE(ss_dim_block(ss_index,4), ierr)
        end if
    enddo
!cpb   end subspaces finalize
#endif SUBSPACES
